...
def ExtractSkills(x):
  #Parainfo={}
  lineList = x.lstrip().split("\n")
  if( len(lineList[0]) == len(x.lstrip())):
    print("len(lineList[0]){} == len(x.lstrip()) {}",len(lineList[0]) , len(x.lstrip()))
    return
  skillList=None
  for line1 in lineList[3:]:
    line1=line1.strip()
    if not skillList is None:
      if(len(line1)<3):
        break
      skillNames = re.split(';|,|\*|\n| |\\\\|/', line1)
    ...
    if re.search("skill",line1.lower()) or re.search("experince",line1.lower()):
      skillList=set()
  if skillList is None:
    skillList=[]
  gskillList.extend(skillList)
  sgskillList = set(gskillList)
  gskillList.clear()
  gskillList.extend(sgskillList)


def ExtractParas(x):

Sheadings=set(headings)
with open(xpath+ "/Headings.txt","w") as FH:
  for s in Sheadings:
...
nonedf = pd.DataFrame(resdf.apply(lambda x: ExtractHeading(x=x['RESUME_TEXT']) , axis=1))
nonedf = pd.DataFrame(resdf.apply(lambda x: ExtractSkills(x=x['RESUME_TEXT']) , axis=1))

ParasDF = pd.DataFrame(resdf.apply(lambda x: ExtractParas(x=x['RESUME_TEXT']) , axis=1))
...
res2 = resdf.head(1)['RESUME_TEXT'][0].splitlines(True)

res2 = resdf['RESUME_TEXT'][2:3]
res2.iloc[0]
print(res2.iloc[0].lower())
res2 = res2.iloc[0]
print(res2)
print(res1)
...
paraList = res2.lstrip().split("\n\n")
total = sum([len(para) for para in  paraList])
avgwords = np.average([len(para.split()) for para in  paraList])
headings = [ para for para in  paraList  if len(para.split()) < (avgwords/2) ]
HEADINGS =  ['About Me', 'Education', 'Work Experience','Skill', 'Language', 'Interests']
if re.search("about",res2.lower()):
  print("Found")
